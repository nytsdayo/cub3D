 ...
 */
static int validate_texture_line(const char *line, t_seen_flags seen_flags,
                                 t_identifier id);

/**
 * @brief RGB識別子の行を検証する
 */
static int validate_rgb_line(const char *line, t_seen_flags seen_flags,
                             t_identifier id);
...
int validate_config(char **input_data, size_t *line_index) {
  ...
  while (input_data[*line_index] != NULL &&
    ...
    if (id >= ID_NO && id <= ID_EA)
      result = validate_texture_line(input_data[*line_index], seen_flags, id);
    else
      result = validate_rgb_line(input_data[*line_index], seen_flags, id);
    if (result != 0)
      return (result);
    (*line_index)++;
  }
