...
### メモリ最適化
現在、`keys[KEY_STATE_SIZE]`（70000要素、約280KB）を使用しているが、これはメモリ効率が悪い。

**理由:**
- Linux環境でのX11 KeySym値（最大65535程度）に対応するため
- CodeRabbit指摘の範囲外アクセス問題を修正するための暫定対応
...
3. **キーコードマッピング関数**
   - KeySym → 0-5のインデックスに変換
   - `keys[6]`の小さな配列で管理

**優先度:** 中（パーサー統合後、データ構造の全体再設計時に対応）
